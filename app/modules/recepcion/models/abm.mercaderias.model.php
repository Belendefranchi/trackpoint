...
function obtenerMercaderias() {
	try {
		$conn = getConnection();
		$stmt = $conn->query("SELECT * FROM recepcion_abm_mercaderias");
...
function obtenerMercaderiaPorId($mercaderia_id) {
	try {
		$conn = getConnection();
		$stmt = $conn->prepare("SELECT * FROM recepcion_abm_mercaderias WHERE mercaderia_id = :mercaderia_id");
...
function mercaderiaExists($codigo) {
	try {
		$conn = getConnection();
		$stmt = $conn->prepare("SELECT codigo FROM recepcion_abm_mercaderias WHERE codigo = :codigo");
...
function crearMercaderia($codigo, $descripcion, $familia, $grupo, $subgrupo, $unidad_medida) {
	...
	try {
		$conn = getConnection();
		$stmt = $conn->prepare("INSERT INTO recepcion_abm_mercaderias (codigo, descripcion, familia, grupo, subgrupo, unidad_medida, creado_por) VALUES (:codigo, :descripcion, :familia, :grupo, :subgrupo, :unidad_medida, :creado_por)");
		$stmt->bindParam(':codigo', $codigo);
...
function eliminarMercaderia($mercaderia_id) {
	try {
		$conn = getConnection();
		$stmt = $conn->prepare("DELETE FROM recepcion_abm_mercaderias WHERE mercaderia_id = :mercaderia_id");
		$stmt->bindParam(':mercaderia_id', $mercaderia_id);
		return $stmt->execute();
	} catch (PDOException $e) {
		// Manejo de errores
...
function editarMercaderia($mercaderia_id, $codigo, $descripcion, $familia, $grupo, $subgrupo, $unidad_medida, $activo) {
	...
	try {
		$conn = getConnection();
		$stmt = $conn->prepare("UPDATE recepcion_abm_mercaderias SET codigo = :codigo, descripcion = :descripcion, familia = :familia, grupo = :grupo, subgrupo = :subgrupo, unidad_medida = :unidad_medida, editado_por = :editado_por, activo = :activo WHERE mercaderia_id = :mercaderia_id");
		$stmt->bindParam(':mercaderia_id', $mercaderia_id);
		$stmt->bindParam(':codigo', $codigo);
		$stmt->bindParam(':descripcion', $descripcion);
		$stmt->bindParam(':familia', $familia);
