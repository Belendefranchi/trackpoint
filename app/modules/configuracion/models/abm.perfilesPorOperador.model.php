...
function obtenerPerfilesPorOperador($operadorId) {
	try {
		$conn = getConnection();
    $stmt = $conn->prepare("SELECT p.id, p.nombre FROM configuracion_abm_perfiles p JOIN configuracion_abm_perfilesPorOperador op ON p.id = op.perfil_id WHERE op.operador_id = :operadorId");
