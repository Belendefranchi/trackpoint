...
<script>
	document.addEventListener('DOMContentLoaded', function () {
			...
			function obtenerPerfilesPorOperador(operadorId) {
					fetch('app/modules/configuracion/ajax/obtener_perfiles_por_operador.php?id=' + operadorId)
			...
			document.getElementById('guardarCambios').addEventListener('click', function () {
					if (selectedOperador) {
							fetch('app/modules/configuracion/ajax/asignar_perfiles_a_operador.php', {
									method: 'POST',
									headers: {
									...
									body: JSON.stringify({
											...
											perfiles: Array.from(selectedPerfiles)
									})
							})
									.then(response => response.json())
									.then(data => {
